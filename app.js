...
const PRODUCTS = [
  ...
  {
    id: "brownie-ddl",
    name: "Porciones de Brownie de DdL",
...
const COMBO_PRODUCTS = PRODUCTS.filter((product) => product.category !== "box");
...
const state = {
  ...
  comboSelections: COMBO_PRODUCTS.reduce((accumulator, product) => {
...
function getProductImageSrc(productId) {
  const imageMap = {
    "alfajor-chocolate": "./img/alfajor de dulce de leche.png",
    "alfajor-maicena": "./img/alfajor de maicena.png.jpeg",
    "brownie-ddl": "./img/brownie con dulce de leche.png",
    lemonies: "./img/lemoine.png",
    "coco-ddl": "./img/coco y dulce de leche.png",
  };
...
function getIndividualItemCount() {
  return state.cart.reduce((sum, item) => {
    const itemCategory = item.category || PRODUCT_MAP[item.id]?.category;
    return itemCategory !== "box" ? sum + item.quantity : sum;
  }, 0);
}

function hasBoxOnlyOrder() {
  return state.cart.length > 0 && state.cart.every((item) => (item.category || PRODUCT_MAP[item.id]?.category) === "box");
}

function getComboSelectedCount() {
  return Object.values(state.comboSelections).reduce((sum, quantity) => sum + quantity, 0);
}

function getComboBasePrice() {
  return COMBO_PRODUCTS.reduce((sum, product) => {
    return sum + product.price * state.comboSelections[product.id];
  }, 0);
}

function getComboDiscount() {
  return COMBO_PRODUCTS.reduce((sum, product) => {
    const quantity = state.comboSelections[product.id];
...
function getComboTotal() {
  return Math.max(0, getComboBasePrice() - getComboDiscount());
}

function getComboSummaryParts() {
  return COMBO_PRODUCTS.filter((product) => state.comboSelections[product.id] > 0).map(
    (product) => `${state.comboSelections[product.id]} ${product.name}`
  );
}
    ...
    <header class="navbar">
      <div class="container navbar-inner">
        <a class="brand" href="#inicio" aria-label="Ir al inicio">
          <img class="brand-logo" src="./img/bienargento_wordmark.svg" alt="BienArgento" />
        </a>
...
function renderHero() {
  ...
  return `
    <section class="hero" id="inicio">
      <div class="container">
        <div class="hero-card">
          <div class="hero-image-layer" aria-hidden="true"></div>
      ...
      <article class="product-card">
        ...
        >
          <div class="product-image">
            ${
              imageSrc
                ? `<img class="product-photo product-photo--${product.id}" src="${imageSrc}" alt="${escapeHtml(product.name)}" />`
          ...
          <div class="product-body">
            ...
            <span class="product-link">Ver detalle</span>
          </div>
        </button>
      </article>
    `
...
function renderShopSection() {
  const items = COMBO_PRODUCTS.map((product) => {
    ...
    return `
      <article class="order-card">
        <div class="order-card-visual">
          ${
            imageSrc
              ? `<img class="order-card-photo" src="${imageSrc}" alt="${escapeHtml(product.name)}" />`
  ...
  const comboCards = COMBO_PRODUCTS.map((product) => {
    const imageSrc = getProductImageSrc(product.id);
    const quantity = state.comboSelections[product.id];
    ...
    return `
      <article class="combo-card">
        <div class="combo-card-image">
          ${imageSrc ? `<img class="order-card-photo" src="${imageSrc}" alt="${escapeHtml(product.name)}" />` : ""}
        </div>
        <div class="combo-card-body">
          <h3 class="combo-card-title">${escapeHtml(product.name)}</h3>
          <p class="combo-card-price">${formatPrice(product.price)}</p>
        </div>
        <div class="combo-card-controls">
          <button
            type="button"
            class="order-adjust"
            data-action="combo-adjust"
            data-product-id="${product.id}"
            data-delta="-1"
            aria-label="Quitar ${escapeHtml(product.name)} del combo"
          >-</button>
          <span class="order-qty">${quantity}</span>
          <button
            type="button"
            ...
            data-product-id="${product.id}"
            data-delta="1"
            aria-label="Agregar ${escapeHtml(product.name)} al combo"
          >+</button>
        </div>
      </article>
    `;
  ...
  return `
    <section id="shop" class="shop-builder">
      <div class="shop-header">
        ...
        <p class="section-copy">
          Elegi tus favoritos, ajusta la cantidad y pasa al checkout de abajo. Tambien podes armar tu Box Degustacion de 6 unidades:
          por cada grupo de 3 iguales descontamos €2, y si elegis 6 iguales descontamos €3 en total.
        </p>
      </div>
      ...
      <div class="combo-builder">
        <div class="combo-builder-header">
          <div>
            <h3 class="combo-builder-title">Box Degustacion: arma tu combo</h3>
            <p class="combo-builder-copy">
              Completá 6 unidades entre los sabores individuales. 3 iguales descuentan €2 y 6 iguales descuentan €3.
            </p>
          </div>
          <div class="combo-builder-status">
            <span>${comboCount}/6 elegidos</span>
            <strong>${formatPrice(comboTotal)}</strong>
          </div>
        </div>
        ...
        <div class="combo-summary">
          <div class="summary-meta">
            <span>Total base: ${formatPrice(getComboBasePrice())}</span>
            <span>Descuento combo: -${formatPrice(comboDiscount)}</span>
            <span>Total box: ${formatPrice(comboTotal)}</span>
          </div>
          <button
            type="button"
            class="button"
            data-action="add-combo"
            ${comboCount !== 6 ? "disabled" : ""}
          >
            Agregar Box Degustacion
          </button>
        </div>
      </div>
    </section>
      ...
      <div class="container footer-shell">
        ...
        <div class="footer-branding">
          <div class="footer-logo-row">
            <img class="footer-logo" src="./img/bienargento_wordmark.svg" alt="BienArgento" />
          </div>
      ...
      <div class="modal-card">
        <div class="modal-layout">
          <div class="modal-image">
            ${
              imageSrc
                ? `<img class="modal-photo" src="${imageSrc}" alt="${escapeHtml(product.name)}" />`
                : `<div class="dessert-badge" aria-hidden="true">${product.emoji}</div>`
            }
          </div>
...
// Lets the user build the Box Degustacion with 6 units and automatic discounts.
function adjustCombo(productId, delta) {
  const currentQuantity = state.comboSelections[productId];
  const currentTotal = getComboSelectedCount();

  if (delta > 0 && currentTotal >= 6) {
    return;
  }

  state.comboSelections[productId] = Math.max(0, currentQuantity + delta);
  renderApp();
}
...
function addComboToCart() {
  ...
  if (comboCount !== 6) {
    state.errors = ["El Box Degustacion debe tener exactamente 6 unidades."];
    state.successMessage = "";
    renderApp();
    return;
  }

  const comboSummary = getComboSummaryParts();
  const signature = COMBO_PRODUCTS.map((product) => `${product.id}:${state.comboSelections[product.id]}`).join("|");
  const comboName = `Box Degustacion (${comboSummary.join(", ")})`;
  const comboPrice = getComboTotal();
  const comboId = `box-degustacion-${signature}`;
  const existingItem = state.cart.find((item) => item.id === comboId);
  ...
  state.comboSelections = COMBO_PRODUCTS.reduce((accumulator, product) => {
    accumulator[product.id] = 0;
    return accumulator;
  }, {});
  state.errors = [];
  state.successMessage = "Box Degustacion agregado al carrito.";
  renderApp();
}
...
document.addEventListener("click", (event) => {
  ...
  if (action === "combo-adjust") {
    adjustCombo(productId, Number(delta));
  }
